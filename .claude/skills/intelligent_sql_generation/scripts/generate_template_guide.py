...
def detect_renderer(template_type: str, skill_root: Path) -> tuple[str, str]:
    sql_renderer = skill_root / "assets" / "templates" / "sql" / f"{template_type}.sql"
    if sql_renderer.exists():
        return "sql", f"assets/templates/sql/{sql_renderer.name}"

    shell_renderer = skill_root / "assets" / "templates" / "shell" / f"{template_type}.sh"
    if shell_renderer.exists():
        return "shell", f"assets/templates/shell/{shell_renderer.name}"
...
def build_prompt_examples(entries: list[TemplateEntry]) -> list[str]:
    prompts = [
    ...
    if "data_diff" in {entry.template_type for entry in entries if entry.category == "sql"}:
        prompts.append("Which template should I use to compare differences between source and target?")

    if "null_rate" in {entry.template_type for entry in entries if entry.category == "sql"}:
        prompts.append("Use the `null_rate` template to generate null-rate SQL for `foo.bar` on `ds='2025-01-01'`.")
    elif sql_template:
        prompts.append(f"Use the `{sql_template}` template to generate SQL for `foo.bar` on `ds='2025-01-01'`.")

    if hdfs_template == "hdfs_du":
        prompts.append("Use the `hdfs_du` template to check the size of `imd_aml_safe.table_a` for `ds='2025-01-01'`.")
...
def build_markdown(entries: list[TemplateEntry]) -> str:
    ...
    prompt_examples = build_prompt_examples(entries)
    ...
    parts.extend(f"- {prompt}" for prompt in prompt_examples)
    parts.extend(
        [
            "",
...
def generate_guide(output_path: Path) -> None:
    ...
    yaml_dir = skill_root / "assets" / "templates" / "yaml"
    entries = sorted(
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default=str(find_skill_root() / "references" / "TEMPLATE_GUIDE.md"),
