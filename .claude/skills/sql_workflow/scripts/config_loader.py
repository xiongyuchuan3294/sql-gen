...
SCENARIO_CONFIG_DIR = SKILL_DIR / "assets" / "scenarios"
EXAMPLE_INPUT_DIR = SKILL_DIR / "assets" / "examples"
OUTPUT_DIR = SKILL_DIR / "output"
INTELLIGENT_SQL_SCRIPT_DIR = SKILL_DIR.parent / "intelligent_sql_generation" / "scripts"
DB_NAME_HINTS = (
    "imd_aml300_ads_safe",
    "imd_amlai_ads_safe",
    "imd_aml_dm_safe",
...
class ScenarioConfig:
    """Scenario definition loaded from assets/scenarios/*.yaml."""
...
class WorkflowEngine:
    ...
    def _extract_join_keys_from_text(user_input: str) -> list[str]:
        ...
        raw = match.group(1)
        for stop_word in ("partition", "workflow", "scenario", "table", "\u5206\u533a", "\u5de5\u4f5c\u6d41", "\u573a\u666f", "\u8868"):
        ...
        normalized = (
            raw.replace("\u548c", ",").replace("\u3001", ",").replace("\uff0c", ",").replace(";", ",").replace("\uff1b", ",")
        )
    ...
    def _resolve_input_yaml_path(yaml_path: Path) -> Path:
        ...
        - `<file>.yaml` shorthand (auto lookup in `assets/examples/` and `output/`)
        """
        ...
        if not candidate.is_absolute():
            probe_paths.append(Path.cwd() / candidate)
            probe_paths.append(SKILL_DIR / candidate)
        ...
        if not candidate.is_absolute() and len(candidate.parts) == 1:
            probe_paths.append(EXAMPLE_INPUT_DIR / candidate.name)
